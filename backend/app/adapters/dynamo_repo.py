...
class DynamoEolCacheRepository(IEolCacheRepository):
            ...
            for item in response.get("Items", []):
                ...
                if expires_at > int(datetime.utcnow().timestamp()):
                    status = EolStatus(
                        ...
                        eol_date=datetime.fromisoformat(item["eol_date"]["S"]) if "eol_date" in item else None,
                        is_eol=item.get("is_eol", {}).get("BOOL", False),
                        last_scanned_at=datetime.fromisoformat(item["last_scanned_at"]["S"])
                    )
    ...
    async def set_eol_status(self, status: EolStatus, ttl_seconds: int = 86400) -> None:
        ...
        async with self.session.create_client('dynamodb', region_name=settings.aws_region) as client:
