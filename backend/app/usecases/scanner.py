...
class FrameworkEolScanner:
    ...
    async def scan_repo(self, repo: Repository) -> List[EolStatus]:
        ...
        if simulate_framework_found == "nuxt" and simulate_version_found.startswith(
            "2."
        ):
            is_eol = True
...
class ScanRepositoryUseCase:
    def __init__(
        self, repo_repository: IRepoRepository, cache_repository: IEolCacheRepository
    ):
    ...
    async def execute(self, org_id: str) -> List[EolStatus]:
        ...
        # Seed mock repository for demo org-1 if there are no repositories
        if not repos and org_id == "org-1":
            import uuid
            mock_repo = Repository(
                id=str(uuid.uuid4()),
                github_id=123456,
                name="demo-frontend",
                full_name="org-1/demo-frontend",
                org_id="org-1",
                owner_login="org-1",
                default_branch="main"
            )
            await self.repo_repository.save(mock_repo)
            repos = [mock_repo]
